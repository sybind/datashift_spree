  ...
  module SpreeEcom
    ...
    class ProductLoader < SpreeBaseLoader
      ...
      def perform_load( file_name, opts = {} )
        ...
        @we_can_process_these_anyway =  ["images","variant_sku","variant_cost_price","variant_price","variant_images","stock_items","au_price","uk_price","active_sale_discount_rate","feedback_count"]
      ...
      def process(method_detail, value)  
        ...
        elsif(current_method_detail.operator?('gm_product_feed_desc') && current_value)        
          if(@load_object.gm_product_feed_desc.to_s)
            @load_object.gm_product_feed_desc = current_value
            @load_object.save
          else
            puts "WARNING: Problem setting Google Merchant product feed description - None Set"
          end
